...
def main():
        ...
        if prob not in count:
            ...
            if sel not in count[prob]:
                ...
                if cn not in count[prob][sel]:
                    count[prob][sel][cn] = [eve]
                else:
                    count[prob][sel][cn].append(eve)
            else:
                if cn not in count[prob][sel]:
                    count[prob][sel][cn] = [eve]
                else:
    ...
    for prob in count.keys():
        ...
        for sel in count[prob].keys():
            ...
            raw_data = {'dims':dims, 'eval':gens}
            df = pd.DataFrame(raw_data, columns=['dims', 'eval'])
            df.to_csv(write_directory+prob+'__'+sel+'__violin__eval.csv')
